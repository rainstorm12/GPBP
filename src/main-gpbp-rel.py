...
optimizer = set_optimizer(gpnet2, train_steps= (int(len(train_data) / con.getint("para", "batch_size")) + 1) * con.getint("para", "epochs"))
# optimizer = torch.optim.AdamW(
# 	net.parameters(),
#     lr=1e-5
# )

def load_entlabels(name):
    itemlist = load_data(name)
    entlablelist = []
    for item in itemlist:
        text = item["text"]
        encoder_text = tokenizer(text, return_offsets_mapping=True, max_length=maxlen, truncation=True)
        input_ids = encoder_text["input_ids"]
        spoes = set()
        for s,s_head,s_tail,p,o, o_head,o_tail, s_type, o_type in item["spo_list"]:
            ...
            if sh != -1 and oh != -1:
                spoes.add((sh, sh+len(s)-1, p, oh, oh+len(o)-1))#头实体head，头实体tail，关系，尾实体head，尾实体tail
        entity_labels = [set() for i in range(len(en2id))]
        for ehead,etail,etype in item["entity_list"]:
            p = en2id[etype]
            e = tokenizer.encode(text[ehead:etail+1], add_special_tokens=False)
            ehlist = search_list(e,input_ids)
            if len(ehlist):
                for eh in ehlist:
                    entity_labels[p].add((eh, eh+len(e)-1)) 
        entlablelist.append(entity_labels)
    return entlablelist

train_true_entl = load_entlabels(args_path["train_file"])
test_true_entl = load_entlabels(args_path["test_file"])
...
train_true_entl = train_true_entl[0:train_eval_size]

test_info = {}
train_info = {}

test_info['label-rel_precision'] = []
test_info['label-rel_recall'] = []
test_info['label-rel_f1'] = []  
test_info['net-rel_precision'] = []
test_info['net-rel_recall'] = []
test_info['net-rel_f1'] = []  

train_info['label-rel_precision'] = []
train_info['label-rel_recall'] = []
train_info['label-rel_f1'] = [] 
train_info['net-rel_precision'] = []
train_info['net-rel_recall'] = []
train_info['net-rel_f1'] = []  
train_info['loss'] = [] 
...
for eo in range(con.getint("para", "epochs")):
    ...
    for idx, batch in enumerate(train_loader):
        ...
        logits1, logits2 = gpnet2(batch_token_ids, batch_mask_ids, batch_token_type_ids)
    ...
    print('\n')
    #train
    ner_precision,ner_recall,ner_f1,rel_precision,rel_recall,rel_f1=\
        utils.target_calculate.gpnet2_test_printInfo2(train_true_entl,train_text_list,train_true_spo,train_true_ent,tokenizer,gpnet2,device,id2schema,id2en)
    print('label-train ner|precision:',ner_precision,'|recall:',ner_recall,'|f1:',ner_f1)
    print('label-train rel|precision:',rel_precision,'|recall:',rel_recall,'|f1:',rel_f1)
    train_info['label-rel_precision'].append(rel_precision)
    train_info['label-rel_recall'].append(rel_recall)
    train_info['label-rel_f1'].append(rel_f1)
    ner_precision,ner_recall,ner_f1,rel_precision,rel_recall,rel_f1=\
        utils.target_calculate.gpnet2_test_printInfo(train_text_list,train_true_spo,train_true_ent,tokenizer,gpnet1,gpnet2,device,id2schema,id2en)
    print('gpnet1-train ner|precision:',ner_precision,'|recall:',ner_recall,'|f1:',ner_f1)
    print('gpnet1-train rel|precision:',rel_precision,'|recall:',rel_recall,'|f1:',rel_f1)
    train_info['net-rel_precision'].append(rel_precision)
    train_info['net-rel_recall'].append(rel_recall)
    train_info['net-rel_f1'].append(rel_f1)
    with open('logdata_train_gpnet2.json', 'w') as f:
        json.dump(train_info, f)
    #test
    ner_precision,ner_recall,ner_f1,rel_precision,rel_recall,rel_f1=\
        utils.target_calculate.gpnet2_test_printInfo2(test_true_entl,test_text_list,test_true_spo,test_true_ent,tokenizer,gpnet2,device,id2schema,id2en)
    print('label-test ner|precision:',ner_precision,'|recall:',ner_recall,'|f1:',ner_f1)
    print('label-test rel|precision:',rel_precision,'|recall:',rel_recall,'|f1:',rel_f1)
    test_info['label-rel_precision'].append(rel_precision)
    test_info['label-rel_recall'].append(rel_recall)
    test_info['label-rel_f1'].append(rel_f1)
    ner_precision,ner_recall,ner_f1,rel_precision,rel_recall,rel_f1=\
        utils.target_calculate.gpnet2_test_printInfo(test_text_list,test_true_spo,test_true_ent,tokenizer,gpnet1,gpnet2,device,id2schema,id2en)
    print('gpnet1-test ner|precision:',ner_precision,'|recall:',ner_recall,'|f1:',ner_f1)
    print('gpnet1-test rel|precision:',rel_precision,'|recall:',rel_recall,'|f1:',rel_f1)
    test_info['net-rel_precision'].append(rel_precision)
    test_info['net-rel_recall'].append(rel_recall)
    test_info['net-rel_f1'].append(rel_f1)
    with open('logdata_test_gpnet2.json', 'w') as f:
        json.dump(test_info, f)
    if rel_f1>best_rel_f1:
        best_rel_f1 = rel_f1
        torch.save(gpnet2.state_dict(), './gpnet2_best.pth')
        print('save best rel model')
    # if rel_f1>best_rel_f1:
    #     best_rel_f1 = rel_f1
    #     torch.save(gpnet1.state_dict(), './erenet_rel_best.pth')
    #     print('save best rel model')
    torch.save(gpnet2.state_dict(), './gpnet2.pth')
